...
public class Ease implements TimeInterpolator {
    ...
    public static org.androidtown.conn.boommenu.Animation.Ease getInstance(EaseEnum easeEnum) {
        if (eases == null) {
            eases = new ArrayList<>(EaseEnum.values().length);
            for (int length = EaseEnum.values().length; length > 0; length--) eases.add(null);
        }
        org.androidtown.conn.boommenu.Animation.Ease ease = eases.get(easeEnum.getValue());
        if (ease == null) {
            ease = new org.androidtown.conn.boommenu.Animation.Ease(easeEnum);
